...
from tool import check_exists, check_punctuation, check_word, create_word_srt
import pysrt
...
def rewrite_text(
    textSrtPath, wordSrtPath, srtPath, rewrite_mode="aurora", overwrite=True
):
    rewriteSrtPath = Path(
        ".".join(
            textSrtPath.as_posix().split(".")[:-3]
            + ["rewrite"]
    ...
    if rewrite_mode == "punctuation":
        ...
        for sub in subs:
            ...
            if (
                check_punctuation(sub.text)
                and check_word(sub.text)
                and len(res[-1][0]) > 6
            ):
                res.append([[], [], []])
