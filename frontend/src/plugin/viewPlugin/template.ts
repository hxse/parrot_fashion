...
import { newType, fileAllType } from '../../components/processData'
import { str2srt, srtType } from '../../components/analyzeSrt'
import { idxDirAtom, dirObjAtom } from '../../components/fetchData'
import { childrenType } from '../../components/processData'
...
export const keyStr: keyType = 'view'
export type keyType = 'view'
...
export type updateType = {
  // idx: Atom<number>
  // setIdx: WritableAtom<number, number, void>
  // idxDir: Atom<number[]>
  // setIdxDir: WritableAtom<number[], number[], void>
  idx: WritableAtom<number, number, void>
  idxDir: WritableAtom<number[], number[], void>
}
export type viewTempConfigType = {
  key: string
  audioSuffix: string
  originSuffix: string
  targetSuffix: string
...
export type serverTempConfigType = {
  key: string
  audioSuffix?: string
  originSuffix?: string
  targetSuffix?: string
  ...
  dirPath?: string
  current?: boolean
} & Partial<updateServerType> &
  Partial<mode>

type configType = {
  dirObj: pathType
  argObj: viewTempConfigType
}
...
import { viewChildrenType } from './children'

export type pluginConfigType = {
  plugin: pluginType
  pluginAtom: audioTempAtomObjType
  pluginArgs: viewTempConfigType
  pluginArgsSave: serverTempConfigType
  PluginChildren: viewChildrenType
}

export const viewPlugin = {
  tempAtomObj: audioTempAtomObj,
  match
}
export type pluginType = typeof viewPlugin

export type serverTempConfigType2 = {
  key: 'hello'
  name: 'world'
  audioSuffix?: string
  originSuffix?: string
...
export type pluginConfigType2 = {
  plugin: pluginType
  pluginAtom: audioTempAtomObjType
  pluginArgs: viewTempConfigType
  pluginArgsSave: serverTempConfigType2
  PluginChildren: viewChildrenType
}
